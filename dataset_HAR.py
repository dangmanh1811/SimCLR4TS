...
print(f"Train Sample: {args.dataset_train['samples'].shape}; Label: {args.dataset_train['labels'].shape}")

args.dataset_val = torch.load('Datasets/HAR/val.pt')
print(f"Val Sample: {args.dataset_val['samples'].shape}; Label: {args.dataset_val['labels'].shape}")

args.dataset_test = torch.load('Datasets/HAR/test.pt')
print(f"Test Sample: {args.dataset_test['samples'].shape}; Label: {args.dataset_test['labels'].shape}")
...
for i in range(n_class):
	print(f"There are {(train_y == i).sum()} samples of Class {i}")
	id = train_y == i
